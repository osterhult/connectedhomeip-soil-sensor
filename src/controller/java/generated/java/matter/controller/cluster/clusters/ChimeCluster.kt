...
class ChimeCluster(private val controller: MatterController, private val endpointId: UShort) {class InstalledChimeSoundsAttribute(
    val value: List<ChimeClusterChimeSoundStruct>
  )
...
suspend fun readInstalledChimeSoundsAttribute(): InstalledChimeSoundsAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Installedchimesounds attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ChimeClusterChimeSoundStruct> = buildList<ChimeClusterChimeSoundStruct> {
  ...
  suspend fun subscribeInstalledChimeSoundsAttribute(
    minInterval: Int,
    maxInterval: Int
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(InstalledChimeSoundsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
      ...
      tlvReader.exitContainer()
          ...
          emit(InstalledChimeSoundsAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(InstalledChimeSoundsAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
suspend fun readActiveChimeIDAttribute(): UByte {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Activechimeid attribute not found in response" 
    }
  ...
  suspend fun writeActiveChimeIDAttribute(
    value: UByte,
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Activechimeid attribute not found in Node State update" 
          }
...
suspend fun readEnabledAttribute(): Boolean {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Enabled attribute not found in response" 
    }
  ...
  suspend fun writeEnabledAttribute(
    value: Boolean,
    timedWriteTimeout: Duration? = null) {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Enabled attribute not found in Node State update" 
          }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(ChimeCluster::class.java.name)
    const val CLUSTER_ID: UInt = 1366u
  }
}
